...
class PlayerState(private val player: Player) {
    ...
    fun update() {
        when (state) {
            State.NOT_MOVING -> if (player.velocityX != 0f || player.velocityY != 0f) state =
                State.START_MOVING
            State.START_MOVING -> if (player.velocityX != 0f || player.velocityY != 0f) state =
                State.IS_MOVING
            State.IS_MOVING -> if (player.velocityX == 0f && player.velocityY == 0f) state =
                State.NOT_MOVING
            else -> {}
        }
    }
